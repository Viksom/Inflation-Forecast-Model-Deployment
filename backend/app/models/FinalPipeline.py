...
PTINFO_PATH = PROJECT_ROOT / "data" / "PTdata.xlsx"
FRED_PATH = PROJECT_ROOT / "data" / "2026-01-MD.csv"
TARGET_COL = "inflation_target"
OUTLIER_PREPROCESSOR_PATH = MODELS_ROOT / "processors" / "standardize_features.pkl"
...
def validate_selected_lags(selected_variables=None, target_col=TARGET_COL, lag_map=None):
    selected_variables = set(selected_variables or SELECTED_VARIABLES)
    selected_variables.add(target_col)
    lag_map = lag_map or SELECTED_LAGS

    invalid_sources = [col for col in lag_map if col not in selected_variables]
    if invalid_sources:
        raise KeyError(f"Lag rules reference variables outside the selected dataset: {invalid_sources}")
    ...
    if invalid_lags:
        raise ValueError(f"Invalid lag rules: {invalid_lags}")
